...
public class TemperatureHumidityToPhoenixSync {
    public static void main(String[] args) throws SQLException {
        ...
        try {
            for (int i = 0; i < 500000; i++) {
                //从mysql中查询出来数据
                String sql = "SELECT * FROM temperature_humidity LIMIT 10000 OFFSET " + i * 10000;
                List<Map<String, Object>> mysqlDataList = fetchMySQLData(mysqlUrl, mysqlUser, mysqlPassword, sql);
                if (mysqlDataList.size() == 0) {
                    break;
                }
                // 插入进Phoenix表
                String upsertStatement = "UPSERT INTO \"air\".\"temperature_humidity\" VALUES (?, ?, ?, ?, ?, ?, ?)";
                ...
                for (Map map : mysqlDataList) {
                    ...
                    upsertStmt.setInt(4, Integer.parseInt(String.valueOf(map.get("location_id"))));
                    upsertStmt.setString(5, String.valueOf(map.get("location_name")));
                    upsertStmt.setDouble(6, Double.parseDouble(String.valueOf(map.get("temperature"))));
                    upsertStmt.setDouble(7, Double.parseDouble(String.valueOf(map.get("humidity"))));
